...
import { listOutputs } from "@/services/knowledge/knowledgeOutputs";
...
const KnowledgeOutputRoute = ({ kind }: { kind: GeneratedOutputKind }) => {
    ...
    if (!outputId) {
        ...
        return <div className="mx-auto w-full max-w-5xl p-4"><h1 className="m-0 text-2xl font-extrabold text-foreground">Session {kind}s</h1><p className="mt-2 text-sm text-muted-foreground">All {kind}s created in this knowledge session.</p>{listError && <p className="rounded-xl bg-danger/10 p-4 text-danger">{listError}</p>}{outputs.length === 0 ? <div className="mt-8 rounded-2xl border border-dashed border-border bg-surface p-10 text-center"><h2 className="m-0 text-lg font-extrabold">No {kind}s in this session</h2><p className="mb-0 mt-2 text-sm text-muted-foreground">Created {kind}s will appear here.</p></div> : <div className="mt-6 grid gap-4 sm:grid-cols-2">{outputs.map((item) => <Link key={item.id} to={`/dashboard/knowledge/${sessionId}/${kind}s/${item.id}`} className="rounded-2xl border border-border bg-surface p-5 text-inherit no-underline shadow-sm transition hover:-translate-y-1 hover:border-primary"><h2 className="m-0 text-base font-extrabold">{item.title}</h2><p className="mt-2 line-clamp-2 text-sm text-muted-foreground">{item.description}</p><div className="mt-4 text-xs font-bold text-primary">{item.schemaCount} sections · {item.status}</div></Link>)}</div>}</div>;
    }
